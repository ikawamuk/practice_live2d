...
export class LAppModel extends CubismUserModel {
   ...
   */
  /**
   * ドラッグ追従(視線・顔・体の向き)に使うパラメータIDを、
   * モデルに実際に存在するIDへ解決する。
   * 標準命名(ParamAngleX)が無ければ旧命名(PARAM_ANGLE_X)を採用する。
   */
  private resolveDragParameterIds(): void {
    const idManager = CubismFramework.getIdManager();
    const pick = (modernId: string, legacyId: string): CubismIdHandle => {
      const modern = idManager.getId(modernId);
      if (this._model.getParameterIndex(modern) >= 0) {
        return modern;
      }
      const legacy = idManager.getId(legacyId);
      if (this._model.getParameterIndex(legacy) >= 0) {
        return legacy;
      }
      return modern;
    };

    this._idParamAngleX = pick(
      CubismDefaultParameterId.ParamAngleX,
      'PARAM_ANGLE_X'
    );
    this._idParamAngleY = pick(
      CubismDefaultParameterId.ParamAngleY,
      'PARAM_ANGLE_Y'
    );
    this._idParamAngleZ = pick(
      CubismDefaultParameterId.ParamAngleZ,
      'PARAM_ANGLE_Z'
    );
    this._idParamBodyAngleX = pick(
      CubismDefaultParameterId.ParamBodyAngleX,
      'PARAM_BODY_ANGLE_X'
    );
    this._idParamEyeBallX = pick(
      CubismDefaultParameterId.ParamEyeBallX,
      'PARAM_EYE_BALL_X'
    );
    this._idParamEyeBallY = pick(
    ...
    if (this._modelSetting.getModelFileName() != '') {
      ...
      fetch(`${this._modelHomeDir}${modelFileName}`)
        ...
        .then(arrayBuffer => {
          ...
          // 旧命名規則(PARAM_ANGLE_X 等)のモデルにも視線追従が効くよう、
          // ドラッグ追従に使うIDをモデルに実在するものへ解決し直す
          this.resolveDragParameterIds();
  ...
  public update(): void {
    ...
    // 風の演出：時間で強弱する「そよ風＋突風」を物理演算に与え、髪などの揺れ物を揺らす。
    // 風は物理演算の力(force = 重力×加速度 + 風)にそのまま加算されるため、
    // おおよそ重力(大きさ1)と同程度の値が体感しやすい。数値を上げるほど強風になる。
    if (this._physics) {
      const t = this._userTimeSeconds;
      const WindDir = -1; // 風向き：-1=右から左へ / +1=左から右へ
      const WindBase = 0.3; // 常に吹いている基本の風の強さ
      const WindGust = 0.9; // 突風で加わる強さ
      // 強弱だけを 0〜1 で変化させる（向きは変えない）。周期の違う波を重ねて不規則な突風にする
      const gust = 0.5 + 0.35 * Math.sin(t * 0.4) + 0.15 * Math.sin(t * 1.3);
      const strength = WindBase + WindGust * gust; // 常に正＝向きは一定
      const wind = this._physics.getOption().wind;
      wind.x = WindDir * strength; // 一定方向に吹き続け、強さだけ揺らぐ横風
      wind.y = 0.1 * Math.sin(t * 2.7); // ごくわずかな上下のあおり
    }
    //--------------------------------------------------------------------
  ...
  /**
   * リップシンク用の音声を再生する。
   * wavハンドラが音量(RMS)を計算してLipSyncパラメータ(口)を動かす。
   * SDKのwavハンドラは音を鳴らさないため、スピーカー再生は別途行う。
   * @param soundFileName モデルディレクトリからの相対パス（例: 'sounds/xxx.wav'）
   */
  public startLipSyncSound(soundFileName: string): void {
    if (this._wavFileHandler == null) {
      return;
    }
    const path = this._modelHomeDir + soundFileName;
    // 口の動き用にRMS計算を開始
    this._wavFileHandler.start(path);
    // 音声再生（自動再生がブロックされても口の動きは継続するため失敗は無視）
  ...
  public constructor() {
    ...
    this._rightArmMotionManager = new CubismMotionManager();
    this._leftArmMotionManager = new CubismMotionManager();
  }

  _rightArmMotionManager: CubismMotionManager;
  _leftArmMotionManager: CubismMotionManager;
