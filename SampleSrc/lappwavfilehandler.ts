...
export class LAppWavFileHandler extends IParameterProvider {
  public update(deltaTimeSeconds?: number): boolean {
    ...
    // 平滑化：ゲイン適用後の目標値へ、フレームごとの急な変化をなめらかにして追従させる。
    // RMSは毎フレーム激しく上下するため、そのまま使うと口が細かく震えて不自然になる。
    const target = Math.min(rms * this.lipSyncGain, 1.0);
    const retain = Math.pow(this.lipSyncSmoothing, actualDeltaTime * 60.0);
    this._smoothedParameter =
      this._smoothedParameter * retain + target * (1.0 - retain);
  ...
  public start(filePath: string): void {
    ...
    // RMS値・平滑化値をリセット
    this._lastRms = 0.0;
    this._smoothedParameter = 0.0;

    this.loadWavFile(filePath);
  }

  /**
   * リップシンク用のゲイン（口の開き具合の倍率）。
   * このモデルの口リグは低い値ではほとんど開かないため、RMSを底上げする。
   */
  public lipSyncGain = 2.8;

  /**
   * 口の動きの平滑化の強さ。60fps基準で「前フレームの値をどれだけ残すか」の割合。
   * 0=平滑化なし（RMSに即追従＝細かく震える）／1に近いほどなめらか（反応は鈍くなる）。
   */
  public lipSyncSmoothing = 0.8;

  /** 平滑化後の口パラメータ値 */
  private _smoothedParameter = 0.0;

  /**
   * Get parameter value for lip sync.
   *
   * @return 平滑化・ゲイン適用済みの口の開き具合（0〜1）
   */
  public getParameter(): number {
    return this._smoothedParameter;
  }
