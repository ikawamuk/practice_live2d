...
export class LAppLive2DManager {
  ...
  public onTap(x: number, y: number): void {
    ...
    if (model.hitTest(LAppDefine.HitAreaNameHead, x, y)) {
      ...
      if (LAppDefine.DebugLogEnable) {
        LAppPal.printMessage(`[APP]hit area: [Right]`);
      }
      model.startRandomRightHandMotion('Right', LAppDefine.PriorityForce);
    } else if (model.hitTest('Left', x, y)) {
      if (LAppDefine.DebugLogEnable) {
        LAppPal.printMessage(`[APP]hit area: [Left]`);
      }
      model.startRandomLeftHandMotion('Left', LAppDefine.PriorityForce);
    } else {
      // ヒットエリアが無い/当たらなかった場合はリップシンク音声を再生する
      // （Ariaのようにモーション・ヒットエリアを持たないモデルでも口パクを確認できる）
      model.startLipSyncSound('sounds/lipsync_sample.wav');
    }
  }

  /**
   * 指定したモーショングループをランダム再生する。
   * キーボード操作など、当たり判定を介さないトリガーから利用する。
   * @param group モーショングループ名（例: FlickUp）
   */
  public startMotionByGroup(group: string): void {
    const model: LAppModel = this._models[0];
    if (model == null) {
      return;
    }
    if (LAppDefine.DebugLogEnable) {
      LAppPal.printMessage(`[APP]motion group: [${group}]`);
    }
    model.startRandomMotion(
      group,
